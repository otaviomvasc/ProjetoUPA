...
def distribuicoes_cen4(processo, slot="None"):
    ...
    dados = {
        ...
        * coef_chegadas
        ...
        "Triagem": max(
            0.6,
            random.weibullvariate(2.526, 2.485),
        )
        * coef_chegadas,
        "Clínico": max(
            4.53,
            random.weibullvariate(6.878, 2.832),
        )
        * coef_chegadas,
        "Pediatra": max(
            5.34,
            random.gauss(14.022, 5.966),
        )
        * coef_chegadas,
...
def calcula_distribuicoes_prob():
    ...
    decisao_apos_medicacao = [
        ...
        [
            "medico",
            0.2,
        ],  # TODO: E O PACIENTE QUE VEM DO PEDIATRA E TOMA MEDICAÇÃO? COMO ELE VAI VOLTAR PARA LÁ?
        ["Raio-x", 0.1],
        ["Eletro", 0.1],
        ["Exame de Urina", 0.1],
        ["Exame de Sangue", 0.1],
...
if __name__ == "__main__":
    ...
    liberacao_recursos = {
        ...
        "Tomar Medicação": [
            "Espaço para tomar Medicação"
        ],  # TODO: pensar em como fazer para liberar apenas 1 requests para liberar apenas a medicação!
        "Eletro": ["Eletro"],
    }
            ...
            dados_wip.append(
                {
                    "Cenário": cen,
                    "Replicação": rep,
                    "WIP": estatisticas_finais[cen][rep]["dados_NS"],
                    "discretizacao": estatisticas_finais[cen][rep]["momento_NS"],
                }
            )
            dados_pacientes_atendidos.append(
                {
                    "Cenário": cen,
                    "Replicação": rep,
                    "entidades_atendidas": len(
                        estatisticas_finais[cen][rep]["dados_TS"]
                    ),
                }
            )
            for rec in estatisticas_finais[cen][rep]["dict_utilizacao"]:
                dados_utilizacao_media.append(
                    {
                        "Cenário": cen,
                        "Replicação": rep,
                        "Recurso": rec,
                        "utilizacao": estatisticas_finais[cen][rep]["dict_utilizacao"][
                            rec
                        ]["dados_utilizacao"],
                        "T": estatisticas_finais[cen][rep]["dict_utilizacao"][rec][
                            "discretizacao"
                        ],
                        "Tempo_fila": estatisticas_finais[cen][rep]["dict_utilizacao"][
                            rec
                        ]["tempo_fila"],
                        "prioridade_entidade": estatisticas_finais[cen][rep][
                            "dict_utilizacao"
                        ][rec]["prioridade_entidade"],
                        "tempo_fila_prioridade_entidade": estatisticas_finais[cen][rep][
                            "dict_utilizacao"
                        ][rec]["tempo_fila_prioridade_entidade"],
                        "processo": estatisticas_finais[cen][rep]["dict_utilizacao"][
                            rec
                        ]["processo"],
                        "tempo_fila_entidades": estatisticas_finais[cen][rep][
                            "dict_utilizacao"
                        ][rec]["tempo_fila_entidades"],
                    }
                )
    ...
    if gera_graficos:
        if traduz:
            ...
            df_recursos = pd.DataFrame(dados_utilizacao_media).explode(
                [
                    "utilizacao",
                    "T",
                    "processo",
                    "prioridade_entidade",
                    "Tempo_fila",
                    "tempo_fila_prioridade_entidade",
                    "tempo_fila_entidades",
                ]
            )
            df_recursos.rename(
                columns={
            ...
            df_recursos["Resource"] = df_recursos.Resource.apply(
                lambda x: dicionario_traduzido_recursos[x]
            )
            df_recursos["Process"] = df_recursos.Process.apply(
                lambda x: dicionario_traduzido_processos[x]
            )

            df_recursos.to_csv("dados_recursos.csv", sep=";")
            # Utilizações:
            # Média geral por cenário!
            df_ocupacao_media_cenario_por_recurso = (
                df_recursos.groupby(by=["Scenarios", "Resource"])
                .agg({"Resources Usage (%)": "mean"})
                .reset_index()
            )

            # Verificar e converter para numérico antes de aplicar round()
            df_ocupacao_media_cenario_por_recurso["Resources Usage (%)"] = (
                pd.to_numeric(
                    df_ocupacao_media_cenario_por_recurso["Resources Usage (%)"],
                    errors="coerce",
                )
            ...
            df_ocupacao_media_cenario_por_recurso["Resources Usage (%)"] = round(
                df_ocupacao_media_cenario_por_recurso["Resources Usage (%)"] * 100, 2
            )
            fig = px.bar(
                df_ocupacao_media_cenario_por_recurso,
                x="Resource",
                ...
                title="Average Utilization Resources in Scenarios",
            )  # text="nation"
            ...
            for s1 in s:
                df = df_ocupacao_media_cenario_por_recurso.loc[
                    df_ocupacao_media_cenario_por_recurso.Scenarios.isin(s1)
                ]
                fig = px.bar(
                    df,
                    x="Scenarios",
            ...
            df_fila_media_por_processo = (
                df_recursos.groupby(by=["Scenarios", "Process"])
                .agg({"Queue Time (Min)": "mean"})
                .reset_index()
            )
            df_fila_entidades = (
                df_recursos.groupby(by=["Scenarios", "Process"])
            ...
            # Verificar e converter para numérico antes de aplicar round()
            df_fila_entidades["Queue Time (Min)"] = pd.to_numeric(
                df_fila_entidades["Queue Time (Min)"], errors="coerce"
            )

            df_fila_entidades["Queue Time (Min)"] = round(
                df_fila_entidades["Queue Time (Min)"], 2
            )
            ...
            fig = px.bar(
                ...
                color="Scenarios",
                text="Queue Time (Min)",
                title="Process Queue Time by Scenarios",
                barmode="group",
            )
            fig.update_traces(texttemplate="%{text:.2s}")
            fig.layout.template = CHART_THEME
            ...
            df_fila_media_prioridade = (
                df_recursos.groupby(by=["Scenarios", "Patient Priority"])
                .agg({"Queue Time (Min)": "mean"})
                .reset_index()
            )
            df_fila_entidades_prioridade = (
                df_recursos.groupby(by=["Scenarios", "Patient Priority"])
                .agg({"Queue Time (Min)": "mean"})
                .reset_index()
            )
            ...
            # Verificar e converter para numérico antes de aplicar round()
            df_fila_entidades_prioridade["Queue Time (Min)"] = pd.to_numeric(
                df_fila_entidades_prioridade["Queue Time (Min)"], errors="coerce"
            )

            df_fila_entidades_prioridade["Queue Time (Min)"] = round(
                df_fila_entidades_prioridade["Queue Time (Min)"], 2
            )
            fig = px.bar(
                df_fila_entidades_prioridade,
                x="Patient Priority",
                ...
                color="Scenarios",
                text="Queue Time (Min)",
                title="Process Queue Patient Priority",
                barmode="group",
            )
            fig.update_traces(texttemplate="%{text:.4}")
            fig.layout.template = CHART_THEME
            ...
            df_aux = df_recursos.loc[
                (
                    (df_recursos.Process == " Clinical Consultation")
                    & (df_recursos["Patient Priority"] == 1)
                )
            ].reset_index()
            df_fila_processo_prioridade_entidade = (
                df_aux.groupby(by=["Scenarios"])
                .agg({"Queue Time (Min)": "mean"})
                .reset_index()
            )  # DF QUE INDICA O KPI DE MÉDIA DE TEMPO DE ESPERA DE PACIENTENTES PRIORIDADE 1 NO CLÍNICO!!
            ...
            df_prioridade_paci_recurso = (
                df_recursos.groupby(by=["Scenarios", "Patient Priority", "Process"])
                .agg({"Queue Time (Min)": "mean"})
                .reset_index()
            )
            fig = px.bar(
            ...
            df_pacientes_atendidos = (
                pd.DataFrame(dados_pacientes_atendidos)
                .groupby(by=["Cenário"])
                .agg({"entidades_atendidas": "mean"})
                .reset_index()
            )
            df_pacientes_atendidos.rename(
                columns={"Cenário": "Scenarios", "entidades_atendidas": "Patient Seen"},
                inplace=True,
            )
            fig = px.bar(
                df_pacientes_atendidos,
